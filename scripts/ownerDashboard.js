document.getElementById("profileLink").addEventListener("click", function () {
  ...
  document
    .getElementById("addPropertyLink")
  ...
  document.addEventListener("DOMContentLoaded", async function () {
    try {
      ...
      if (response.ok) {
        ...
        const editPropertyDropdown = document.getElementById(
      ...
      try {
        ...
        if (response.ok) {
          ...
          property.roomFacilities.forEach((facility) => {
            const facilityCheckbox = document.getElementById(
    ...
    function updateFileCountNextToButton() {
      ...
      const countText =
        selectedFilesCount > 0
          ? `${selectedFilesCount} image${plural}`
          : "No image selected";
  ...
  document.addEventListener("click", async function (event) {
    if (
      event.target.classList.contains("list-btn") ||
      event.target.classList.contains("unlist-btn")
    ) {
      ...
      try {
        const response = await fetch(
          `/owner/dashboard/updateAvailability/${propertyId}`,
          {
            method: "POST",
            headers: {
  ...
  // JavaScript code to toggle edit section visibility and populate edit form
  document.addEventListener("click", function (event) {
    if (event.target.classList.contains("edit-property-btn")) {
      const propertyId = event.target.dataset.propertyId;
      const propertyName = event.target.dataset.propertyName; // Get property name from dataset
      // Populate edit form with property details
      document.getElementById("editPropertyName").value = propertyName || "";
      // Show the edit section
      document.getElementById("editSection").style.display = "block";
      // Set the propertyId as a data attribute or in a hidden input field in the edit form
      document.getElementById("propertyIdInput").value = propertyId;
    }
  });
  
  // JavaScript code to handle form submission for editing property details
  document
    .getElementById("edit-property-form")
    .addEventListener("submit", async function (event) {
      event.preventDefault(); // Prevent default form submission
      const propertyId = document.getElementById("propertyIdInput").value; // Retrieve propertyId from the form
      const formData = new FormData(this); // Get form data
      try {
        const response = await fetch(
          `/owner/dashboard/editProperty/${propertyId}`,
          {
            method: "POST",
            body: formData,
          }
